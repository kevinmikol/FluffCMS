...
while($row = $navigation->fetch(PDO::FETCH_ASSOC)){
	?>
	<script>
		$('.navigationEdit').prepend('<li id="list_<?=$row['item_id']?>" data-url="<?=$_POST['url']?>"><div><span><?=$_POST['text']?></span><a style="color:#da4f49" data-id="<?=$row['item_id']?>" data-title="<?=$row['text']?>" data-type="link" class="deleteButton"><i class="fa fa-trash pull-right"></i></a><a style="color:#faa732" data-id="<?=$row['item_id']?>" class="navitemedit"><i class="fa fa-edit pull-right"></i></a></div></li>');
	</script>
	<?
}
?>
